...
class DamodaranIndustry(Base):
    ...
    avg_effective_tax_rate: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
    avg_debt_to_equity: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
    avg_operating_margin: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
    avg_roc: Mapped[Optional[str]] = mapped_column(Numeric(8, 4), nullable=True)
    avg_reinvestment_rate: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
    cost_of_capital: Mapped[Optional[str]] = mapped_column(Numeric(8, 4), nullable=True)
    fundamental_growth_rate: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
...
class CountryRiskPremium(Base):
    ...
    equity_risk_premium: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
    country_risk_premium: Mapped[Optional[str]] = mapped_column(
        Numeric(8, 4), nullable=True
    )
    updated_at: Mapped[Optional[str]] = mapped_column(
        DateTime(timezone=True), nullable=True
...
class SectorMapping(Base):
    ...
    damodaran_industry_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("damodaran_industries.id"), nullable=True
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "twelvedata_sector",
            "twelvedata_industry",
            name="uq_sector_mapping_composite",
        ),
        Index(
            "ix_sector_mapping_composite", "twelvedata_sector", "twelvedata_industry"
        ),
    )
...
class DcfAuditLog(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    dcf_valuation_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("dcf_valuations.id"), nullable=False, index=True
    )
